...
class io_snake extends IO {
    constructor(body, opts = {}) {
        ...
        this.waveFrequency = opts.frequency ?? 1;
        ...
        this.body.x += Math.cos(this.body.velocity.direction * this.waveFrequency) * this.body.size * Config.bulletSpawnOffset + 0;
        this.body.y += Math.sin(this.body.velocity.direction * this.waveFrequency) * this.body.size * Config.bulletSpawnOffset + 0;
    ...
    think(input) {
        ...
        // Rotate the sin wave
        let trueWaveX = Math.cos(this.waveAngle * this.waveFrequency) * waveX - Math.sin(this.waveAngle * this.waveFrequency) * waveY;
        let trueWaveY = Math.sin(this.waveAngle * this.waveFrequency) * waveX + Math.cos(this.waveAngle * this.waveFrequency) * waveY;
        // Follow the sin wave
...
class io_dieOnMasterRMB extends IO {
    constructor(body) {
        super(body)
    }
    think(input) {
        if (this.body.master.control.alt) {
...
class io_grow extends IO {
    constructor(body, opts = {}) {
        super(body)
        this.increaseAmount = opts.increase ?? 2.5
    }
    think(input) {
        this.body.SIZE += this.increaseAmount
    }
}

let ioTypes = {
    //misc
    DOMRMB: io_dieOnMasterRMB,
    grow: io_grow,
    zoom: io_zoom,
